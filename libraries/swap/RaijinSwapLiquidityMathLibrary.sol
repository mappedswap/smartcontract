...
import "./RaijinSwapLibrary.sol";

// library containing some math for dealing with the liquidity shares of a pair, e.g. computing their exact value
// in terms of the underlying tokens
library RaijinSwapLiquidityMathLibrary {
    ...
    function getReservesAfterArbitrage(
        ...
        (reserveA, reserveB) = RaijinSwapLibrary.getReserves(factory, tokenA, tokenB);

        require(reserveA > 0 && reserveB > 0, "RaijinSwapArbitrageLibrary: ZERO_PAIR_RESERVES");
        ...
        if (aToB) {
            uint256 amountOut = RaijinSwapLibrary.getAmountOut(amountIn, reserveA, reserveB);
            reserveA += amountIn;
            reserveB -= amountOut;
        } else {
            uint256 amountOut = RaijinSwapLibrary.getAmountOut(amountIn, reserveB, reserveA);
            reserveB += amountIn;
            reserveA -= amountOut;
        }
    }
    ...
    function getLiquidityValue(
        ...
        (uint256 reservesA, uint256 reservesB) = RaijinSwapLibrary.getReserves(factory, tokenA, tokenB);
        IUniswapV2Pair pair = IUniswapV2Pair(RaijinSwapLibrary.pairFor(factory, tokenA, tokenB));
